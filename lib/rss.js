...
//   1. Korean Google News (hl=ko, gl=KR) with Korean queries
//   2. English Google News (hl=en-US, gl=US) with English queries
...
// Per-category sources. Each category mixes Korean + English + direct feeds
// so the digest captures both domestic coverage and international trends.
const SOURCES = {
  dev: {
    queriesKo: [
      '소프트웨어 개발 트렌드',
...
const PER_CATEGORY_LIMIT = 30;  // raw cap before summarize trims
...
function cleanDescription(html) {
  ...
  s = s
    ...
    .replace(/\s+/g, ' ')
    .trim();
  // Strip Google News "View Full Coverage" boilerplate
...
async function pullAll(urls, originHint) {
  const out = [];
    ...
    try {
      ...
      for (const it of items) {
        out.push({
          ...
          link: extractLink(it),
          source: extractSource(it) || originHint,
...
export async function fetchCategoryItems(catId, limit = PER_CATEGORY_LIMIT) {
  ...
  const [ko, en, feeds] = await Promise.all([
    pullAll(koUrls, ''),
    pullAll(enUrls, ''),
    pullAll(feedUrls, ''),
  ]);

  const all = [...feeds, ...en, ...ko]; // direct feeds first (richer descriptions)

  // Dedupe by canonical link (strip query strings & utm)
  const canon = (u) => {
    try {
      const x = new URL(u);
      x.search = '';
